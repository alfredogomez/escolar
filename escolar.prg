...
IF (FILE("\Sistemas\sistemas.exe") AND FILE("\Escolar\escolar.exe") AND !esOficinas)
  * No se correrá el controlador de versiones si la base de datos de oficinas generales está usándose.
  * Sólo correra el controlador de versiones si se está en el cpu donde está instalado el servidor. 
  * No se ejecutará si se está en un cpu corriendo el sistema de manera remota ( LAN ).
...
* --------------------------------------
  IF esPlantel
    ...
    * El proceso de mantenimiento sólo se ejecutará en la base de datos del plantel, siguiendo
    * la política de que tanto en coordinación como en oficinas se tenga exactamente lo mismo que
    * en plantel... por lo tanto coordinación y oficinas no deben alterar nada.
    * ¡Sólo en plantel se pueden hacer cambios a la base de datos! el sistema en coordinaciones y
    * en oficinas generales debe reflejar exactamente lo que está en el plantel, ya sea que en plantel
    * esté bien o esté mal la información.
